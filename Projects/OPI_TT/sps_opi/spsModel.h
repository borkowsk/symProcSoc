...
extern bool UzyjGrafiki;//=1; //Parametr konfiguracyjny   USEGRAPH
...
extern unsigned DefLiczbaKrokow;//=0xfffffffe; //Parametr  MAXSTEP
extern unsigned DefCzestoscPomiarow;//=1; //Parametr VIEWFREQ - jak czêsto siê pokazuje i zapisuje do pliku danych
extern unsigned DefLiczbaEksperymentow;//=1;   //Normalnie jeden eksperyment na program   (parametr EXPREPET)
...
class Swiat//:public ElementModelu - PO CO?
{
	...
	static bool UsunWszystko();    //Usuniecie ca³ej zawartoœci, ¿eby mo¿na by³o od nowa
	...
	static unsigned NumerRepetycji();//Która symulacja w tym przebiegu programu
	static void KolejnaSymulacja();// Przygotowuje do kolejnego przebiegu, zeruje numer kroku

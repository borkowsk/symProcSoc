...
// Bez wodotrysk�w, �eby nie inwestowa� zanadto w kosmetyk�, kt�ra ju� mog� zrobi�
...
#include "spsModel.h" //Najwa�niejsze deklaracje konieczne do wsp�pracy modu��w
#include "spsGenLink.h" //Ze wzgl�du na statyczne parametry wizualizacji link�w
...
extern char PlikWejsciowy[256];//="ModelDefaults.dat"; //Wej�ciowy plik struktury (dat)
extern char DelimiterDanych;//='\t'; //Jaki znak s�u�y do rozdzielania danych w pliku DAT (mo�e by� te� : ; , | \ / itp

//Lokalne funkcje strukturalizuj�ce proto-interfejs programu
bool obsluz_mouse_click(); //Jak true to trafiony i trzeba zastopowa� symulacje
bool symuluj=false;
...
void ZmienWidocznoscObiektu(const char* NazwaKlasy, unsigned int MenuId, ssh_menu_handle MainMenu)
{
	...
	FL&=~ElementModelu::WirtualnyKonstruktor::VFLAG; //Wyzerowanie cz�ci wizualnej
	FL|=VF;		 //Naniesienie nowej cz�ci wizyjnej
...
cout<<_LPL("    wykonana na zamowienie OPI przez        \n",
		   "           made for OPI by                  \n");
//cout<<_LPL("  O�rodek Badania Uk�ad�w Z�o�onych ISS UW  \n",
...
czytaj_parametry(argc,argv); //Odczytanie parametr�w wywo�ania innych ni� zaczynaj�ce si� od -
...
//Ustawienie parametr�w okna wizualizacji i jego otwarcie
...
shell_setup(_LPL("TRANSFER TECHNOLOGII w SPS-Frame","TECHNOLOGY TRANSFER in SPS-Frame"),argc,argv); //Szuka parametr�w symshella (tych zaczynaj�cych si� od - )
...
else
{
    ...
    MarkujWszystkieTypy(); //Markery przy wszystkich typach w menu, bo domyślnie są widoczne
}

//if(!testy_graficzne()) {cerr<<endl<<"Nieudane testy grafiki!";goto FINAL;}
//Tak naprawdę zaraz po inicjalizacji - czyli krok 0
...
{ int cont=1; //Ze względu na GOTO nie może być bez określenia zasięgu
ssh_menu_handle MainMenu=ssh_main_menu(); // Daje uchwyt do głównego menu
...
while(cont) //Petla zdarzen na czas pracy algorytmu
{
	if(input_ready())
	{
		...
		case _FASTER_:          //	40030		//MENUITEM "Faster   'f'"
			...
			if(czekaj_ms>0)
			{
				czekaj_ms/=2; //Czas przez p�
			}
			else
			{
				 co_ile_krokow_wizualizacja*=2;
			}
			//cout. ioflags   //Z jakiego� powodu (ustawienia flag?) bez rzutu na double wy�wietla b��dnie
		...
		case 27:symuluj=!symuluj;
			 ...
			 ssh_realize_menu(MainMenu);
			 break;
		case _SPS_CLEAN_ ://Czy�� z menu jest dwuznaczne wi�c nie tylko prze�acza
				Swiat::Ten.Wymazuj();//Ale i czy�ci
	...
	if(symuluj)
	{
	   ...
	   if(
		...
		{
		  //OK - mo�na potwierdzi�                                                      unsigned =0;
		  flush_plot();
		...
		{
			//Licznik krok�w jest w �rodku
			flush_plot();
			cerr<<endl<<"Awaria wy�wietlania po kroku "<<Swiat::NumerKroku()<<endl;
			goto FINAL;
		}
	}
}} //while(cont)

close_plot(); //Normalne zamkni�cie grafiki
...
void MarkujWszystkieTypy()
{
	ssh_menu_handle MainMenu=ssh_main_menu(); // Daje uchwyt do g��wnego menu
...
//R�ne  testy graficzne przed w�a�ciwym modelem

...
///
/// \details
///      Wyodrębnione, żeby można było łatwo zmienić na bardziej zaawansowane środowisko graficzne.
///     ## (c)
...
// Skale, czyli przez ile mnożyć X i Y, żeby wypełnić cały ekran?
// /////////////////////////////////////////////////////////
...
ElementModelu* Swiat::Inspekcja(int x,int y,bool Wizual)
{
    ...
    for(unsigned i=0;i<Swiat::IleWezlow();i++)
    {
       ...
       if(Pom==NULL)
           continue; //Może być pusty slot, który pomijamy
    ...
    if(Wizual)  //TERAZ WŁAŚCIWA PRACA ...
    {
        ...
        for(unsigned j=0;j<Szukany->IlePolDanych();j++)
        {
            ...
            if(WartoPola)
                cout<<WartoPola;
            cout<<endl;
            //Trochę toporne wyświetlanie "ramki" z danymi
...
bool Swiat::Wizualizacja_po_kroku()
{
    ...
    // Teraz już na pewno struktury danych pomocniczych są aktualne
    StrukturaSwiataNieZmieniona=true;

    // Rysowanie węzłów z listy i ich procesów
    for(unsigned i=0;i<IleWezFakt;i++)
    {
        if(WyswietlajWezly)
        {
          if(ListaWez[i]->VWidocznosc()>1) //Węzły na razie tylko widoczne/niewidoczne
             ListaWez[i]->Narysuj();
        }
...
void PowiazanieParaboliczne::Narysuj()
{
    ...
    {                         assert(NumerKoloruTla>255); //Tylko szarości!
        double tlo=(NumerKoloruTla-255)*(1-Waga);
        R=Col.R*Waga+tlo; //Średnie ważone z tłem dają kolory
        G=Col.G*Waga+tlo; //coraz bardziej zbliżone do szarego tła
        B=Col.B*Waga+tlo; //im mniejsza waga
    ...
    if(this->Kierunkowy()) //Jak kierunkowy to trzeba dodać strzałki
    {
       ...
       double Rad=atan2(dY,dX)+M_PI/2.0;  // Nie działa poprawnie bez tej korekty
       Wielobok Arr=Wielobok::Namiot();         //	(3,grubość*1.5);
       Arr.Centruj();
       double MaX,MiX,MaY,MiY,cR,Skala;
       Arr.Zakresy(MiX,MiY,MaX,MaY,cR);
...
void GeneryczneInfo::Wymazuj()
{
    ...
    // Kolorowanie
    set_pen_rgb(NumerKoloruTla-255,NumerKoloruTla-255,NumerKoloruTla-255,0,SSH_LINE_SOLID);
    ...
    // Grubość zależy od wagi i domyślnego R. Ale tu branego "przez p�"
    double Rad=((JakieRwProcWidth/800)*ver)*min(SkalaX,SkalaY);
    if(Rad<1) Rad=1;
...
void GenerycznyProces::Narysuj()
{
    WezelSieci* Mw=Swiat::Wezel(Procesor());
    if(Mw==NULL)
        cerr<<endl<<"Proces na nieistniejącym węźle"<<endl;
    unsigned ile=Swiat::IleProcesow(Procesor());

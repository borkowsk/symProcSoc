...
// //////////////////////////////////////////////////////////////////////////////
//
// Funkcje inicjalizacja struktur danych �wiata  na podstawie pliku
...
static wbrtm::TabelaTabDelimited DaneStruktury;//Dane struktury
										//istniej� ca�y czas, cho� bezpo�rednio
										//nie s� dost�pne
...
{   //WCZYTANIE  Z PLIKU
	...
	bool Udane=DaneStruktury.WczytajZPliku(PlikWejsciowy,DelimiterDanych);//char PlikWejsciowy[] - Wej�ciowy plik struktury (dat)
																//char DelimiterDanych - Jaki znak s�u�y do rozdzielania danych w pliku DAT (mo�e by� \t : ; , | \ / itp
	...
	unsigned Wierszy=DaneStruktury.IleWierszy();  //Liczba wierszy b�dzie jeszcze wiele razy potrzebna
	unsigned Kolumn=DaneStruktury.IleKolumn();	  //Liczba kolumn te� pewnie kilka razy...
	//SPRAWDZENIE ILE JEST WEZ��W  A ILE LINK�W
	...
	//unsigned DefMaxLiczbaWezlow=2;  //Liczby od 1 do 10 oznaczaj� wielokrotno�� liczby pocz�tkowej...
	//unsigned DefMaxLiczbaLinkow=-1;  //wi�ksze s� brane literalnie. -1 dla link�w oznacza graf pe�ny
	//unsigned MaxLiczbaKomunikatow=10*1024; //Ile mo�e jednoczesnie istniec komunikat�w w systemie
	...
	if(0 < DefMaxLiczbaLinkow && DefMaxLiczbaLinkow<=10)
		...
		else
		{
			if(DefMaxLiczbaLinkow==unsigned(-1))
			  DefMaxLiczbaLinkow=Swiat::IleMoznaWezlow()*Swiat::IleMoznaWezlow(); //Mog� by� r�zne rodzaje, lepiej z zapasem
	...
	for(unsigned i=0;i<Wierszy;i++)
	{
		...
		if(strlen(NazwaTypu->c_str())<1) //Pusta kom�rka typu!!!
		{
			cerr<<_LPL("Pusta kom�rka typu. Kolumna A, wiersz ","Empty type definition. Column A, row ")<<i+1<<endl;
			TLOG(0, << _LPL("Pusta kom�rka typu. Kolumna A, wiersz ","Empty type definition. Column A, row ")<<i+1     )
			continue;
		}
		RECOVERY:
		if(*(NazwaTypu->c_str())=='#')  //Komentarz, #config lub definicja nazw p�l
		{
			...
			//Definicja nazw p�l dla jakiego� typu
			const char* pom;
			if((pom=strchr(NazwaTypu->c_str(),'*'))!=NULL)
			 ...
			 {//JEST DEFINICJA
			  ...
			  if(Kon)
			  {
				if(Kon->IleNazwPol()>0 && strcmp(Kon->NazwaPola(1),"<DEF>")!=0) //Ju� co� by�o
					cerr<<_LPL("Powt�rzona definicja nazw kolumn. Wiersz:",
			...
			if(Kon!=NULL) //Jest taki typ
			{
				...
				if((PomW=dynamic_cast<WezelSieci*>(Pom))!=NULL)
				   ...
				   if(!Pom->Poprawny())  //Ostateczne sprawdzenie
						{
							cerr<<i+1<<". wiersz. Obiekt \""<<NazwaTypu->c_str()<<"\" jednak nie jest poprawny, sprawd� dozwolone zakresy warto�ci"<<endl;
   ...
   //	cerr<<" OK"<<endl;
	...
	Swiat::UwagaZmienionoStrukture(); //I tak wykonanie tej funkcji drugi raz w programie mo�e zrobi� kaszan�, ale moze kiedy� ...
	return true;
}
